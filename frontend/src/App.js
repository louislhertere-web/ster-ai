      ...
      <div className="app">
        ...
        <div className="sidebar">
          ...
          <div className="sidebar-section" style={{ marginTop: '8px' }}>
            <div className="sidebar-label">Compétitions</div>
            <div className="nav-item"><span>①</span> National 1</div>
            <div className="nav-item"><span>②</span> National 2</div>
            <div className="nav-item"><span>③</span> National 3</div>
          </div>
            ...
            <div className="filter-pills">
              {['tous', 'rouge', 'jaune', 'vert'].map(f => (
                <button
                  ...
                  onClick={() => setFiltre(f)}
                >
                  {f === 'tous' ? 'Tous' : f === 'rouge' ? 'Prioritaire' : f === 'jaune' ? 'À vérifier' : 'RAS'}
                </button>
              ))}
            </div>
            <button className="btn-recap" onClick={envoyerRecap} disabled={envoi || loading}>
              {envoi ? 'Envoi...' : 'Envoyer récap'}
            </button>
          </div>
            ...
            <div className="stats-bar">
              ...
              <div className="stat-item" onClick={() => setFiltre('jaune')}>
                ...
                <div>
                  ...
                  <div className="stat-lbl">À vérifier</div>
                </div>
              </div>
            ...
            {!loading && !erreur && (
              <>
                ...
                {matchsFiltres.length === 0 && (
                  <div className="state-box">
                    <div className="state-title">Aucun rapport dans cette catégorie</div>
                  </div>
                )}
                ...
                {matchsFiltres.map((m, i) => {
                  ...
                  const label = priorite === 'rouge' ? 'Prioritaire' : priorite === 'vert' ? 'RAS' : 'À vérifier';
                  const icon = priorite === 'rouge' ? '!' : priorite === 'vert' ? '✓' : '~';
